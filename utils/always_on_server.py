    ...
    while time.time() - start_time < timeout:
        try:
            if not TEST_EXECUTION_LOCK.exists():
                # CRITICAL FIX: Server should NEVER create a test execution lock with its own PID
                # Only pytest processes should create test locks
                # The server just checks for locks, but pytest creates them when it runs
                # So we return True here (no lock exists, test can run)
                # The pytest process will create its own lock when it starts
                print(f"OK: No test execution lock - test can proceed (pytest will create lock)")
                return True
            else:
...
def run_test_command(test_name):
    ...
    # IMPORTANT: The server process should NEVER create a test execution lock
    # Only the pytest subprocess should create the lock
    # We check for locks but don't create one here - pytest will create it
    ...
    try:
        ...
        time.sleep(5)  # Increased to 5 seconds to ensure log files are fully flushed and written
        ...
        # Clean up any stale locks (but don't release if we didn't create it)
        # The pytest process should have cleaned up its own lock
        try:
            if TEST_EXECUTION_LOCK.exists():
                lock_pid = TEST_EXECUTION_LOCK.read_text().strip()
                server_pid = os.getpid()
                ...
                if lock_pid == str(server_pid):
                    print(f"WARNING: Removing invalid test lock after test completion (server PID)")
                    TEST_EXECUTION_LOCK.unlink()
                # If lock is old (>10 min), remove it (stale)
                elif (time.time() - TEST_EXECUTION_LOCK.stat().st_mtime) > 600:
                    print(f"WARNING: Removing stale test lock after test completion (age > 10 min)")
                    TEST_EXECUTION_LOCK.unlink()
        except Exception:
            pass
...
def process_queue():
    ...
    # Check if queue is locked, but allow stale locks to be removed
    if is_locked():
    ...
    try:
        while queue:
            ...
            if queue:
                time.sleep(1)
        print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ========================================")
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Queue processing completed")
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ========================================")
    except Exception as e:
                    ...
                    try:
                        ...
                        if lock_pid == str(server_pid):
                            try:
                                ...
                                pass
                            is_busy = False
                        # Basic check if file is fresh (less than 5 mins) and not server PID
                        elif lock_age < 300:
    ...
    def do_POST(self):
        ...
        if self.path == '/add-test':
            try:
                ...
                # Process queue in background (queue watcher will also pick it up, but trigger immediately)
                def process_in_background():
                    time.sleep(0.5)  # Small delay to ensure file is written
                    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Background thread triggered to process queue")
                    process_queue()
                
                thread = threading.Thread(target=process_in_background, daemon=True)
                thread.start()
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Background processing thread started (will process queue in 1 second)")
...
""")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] You can test the server by visiting: http://localhost:{port}/")
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Status endpoint: http://localhost:{port}/status")
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Add test endpoint: http://localhost:{port}/add-test")
    print()
